...
abstract class BasePhoto extends BaseObject  implements Persistent {
	...
	/**
	 * @var        array Like[] Collection to store aggregation of Like objects.
	 */
	protected $collLikes;

	/**
	 * @var        Criteria The criteria used to select the current contents of collLikes.
	 */
	private $lastLikeCriteria = null;
	...
	/**
	 * @var        array UserPhoto[] Collection to store aggregation of UserPhoto objects.
	 */
	protected $collUserPhotos;

	/**
	 * @var        Criteria The criteria used to select the current contents of collUserPhotos.
	 */
	private $lastUserPhotoCriteria = null;
	...
	public function setThemeId($v)
	{
		...
		if ($this->theme_id !== $v) {
			$this->theme_id = $v;
			$this->modifiedColumns[] = PhotoPeer::THEME_ID;
		}
	...
	public function setPath($v)
	{
		...
		if ($this->path !== $v) {
			$this->path = $v;
			$this->modifiedColumns[] = PhotoPeer::PATH;
		}
	...
	public function setIsActive($v)
	{
		...
		if ($this->is_active !== $v || $this->isNew()) {
			$this->is_active = $v;
			$this->modifiedColumns[] = PhotoPeer::IS_ACTIVE;
		}
	...
	public function setViews($v)
	{
		...
		if ($this->views !== $v || $this->isNew()) {
			$this->views = $v;
			$this->modifiedColumns[] = PhotoPeer::VIEWS;
		}
	...
	public function setComments($v)
	{
		...
		if ($this->comments !== $v || $this->isNew()) {
			$this->comments = $v;
			$this->modifiedColumns[] = PhotoPeer::COMMENTS;
		}
	...
	public function setLocation($v)
	{
		...
		if ($this->location !== $v) {
			$this->location = $v;
			$this->modifiedColumns[] = PhotoPeer::LOCATION;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->theme_id = ($row[$startcol + 1] !== null) ? (int) $row[$startcol + 1] : null;
			...
			$this->comments = ($row[$startcol + 7] !== null) ? (int) $row[$startcol + 7] : null;
			...
			return $startcol + 11; // 11 = PhotoPeer::NUM_COLUMNS - PhotoPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		if ($deep) {  // also de-associate any related objects?
			...
			$this->aTheme = null;
			$this->collLikes = null;
			$this->lastLikeCriteria = null;

			$this->collPhotoComments = null;
			$this->lastPhotoCommentCriteria = null;

			$this->collUserPhotos = null;
			$this->lastUserPhotoCriteria = null;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->collLikes !== null) {
				foreach ($this->collLikes as $referrerFK) {
			...
			if ($this->collUserPhotos !== null) {
				foreach ($this->collUserPhotos as $referrerFK) {
					if (!$referrerFK->isDeleted()) {
						$affectedRows += $referrerFK->save($con);
					}
				}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(PhotoPeer::THEME_ID)) $criteria->add(PhotoPeer::THEME_ID, $this->theme_id);
		if ($this->isColumnModified(PhotoPeer::TITLE)) $criteria->add(PhotoPeer::TITLE, $this->title);
		if ($this->isColumnModified(PhotoPeer::PATH)) $criteria->add(PhotoPeer::PATH, $this->path);
		if ($this->isColumnModified(PhotoPeer::DESCRIPTION)) $criteria->add(PhotoPeer::DESCRIPTION, $this->description);
		if ($this->isColumnModified(PhotoPeer::IS_ACTIVE)) $criteria->add(PhotoPeer::IS_ACTIVE, $this->is_active);
		if ($this->isColumnModified(PhotoPeer::VIEWS)) $criteria->add(PhotoPeer::VIEWS, $this->views);
		if ($this->isColumnModified(PhotoPeer::COMMENTS)) $criteria->add(PhotoPeer::COMMENTS, $this->comments);
		if ($this->isColumnModified(PhotoPeer::LOCATION)) $criteria->add(PhotoPeer::LOCATION, $this->location);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		if ($deepCopy) {
			...
			foreach ($this->getLikes() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addLike($relObj->copy($deepCopy));
				}
			}
			...
			foreach ($this->getUserPhotos() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addUserPhoto($relObj->copy($deepCopy));
				}
			}
	 ...
	 * @throws     PropelException
	 */
	public function getTheme(PropelPDO $con = null)
	{
		if ($this->aTheme === null && ($this->theme_id !== null)) {
			$this->aTheme = ThemePeer::retrieveByPk($this->theme_id);
			/* The following can be used additionally to
			   guarantee the related object contains a reference
			   to this object.  This level of coupling may, however, be
			   undesirable since it could result in an only partially populated collection
			   in the referenced object.
			   $this->aTheme->addPhotos($this);
			 */
		}
		return $this->aTheme;
	}
	...
	/**
	 * Clears out the collLikes collection (array).
	 *
	...
	{
		$this->collLikes = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collLikes collection (array).
	 *
	 * By default this just sets the collLikes collection to an empty array (like clearcollLikes());
	...
	{
		$this->collLikes = array();
	}

	/**
	 * Gets an array of Like objects which contain a foreign key that references this object.
	 *
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Photo has previously been saved, it will retrieve
	 * related Likes from storage. If this Photo is new, it will return
	 ...
	 * @throws     PropelException
	 */
	public function getLikes($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(PhotoPeer::DATABASE_NAME);
		}
		...
		if ($this->collLikes === null) {
			if ($this->isNew()) {
			   $this->collLikes = array();
				...
				$criteria->add(LikePeer::PHOTO_ID, $this->id);

				LikePeer::addSelectColumns($criteria);
				$this->collLikes = LikePeer::doSelect($criteria, $con);
			}
		} else {
			...
			if (!$this->isNew()) {
				...
				LikePeer::addSelectColumns($criteria);
				if (!isset($this->lastLikeCriteria) || !$this->lastLikeCriteria->equals($criteria)) {
					$this->collLikes = LikePeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastLikeCriteria = $criteria;
		return $this->collLikes;
	}

	/**
	 * Returns the number of related Like objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related Like objects.
	 * @throws     PropelException
	 */
	public function countLikes(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(PhotoPeer::DATABASE_NAME);
		} else {
		...
		if ($this->collLikes === null) {
			if ($this->isNew()) {
				...
				$count = LikePeer::doCount($criteria, false, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
	...
	/**
	 * Method called to associate a Like object to this object
	 * through the Like foreign key attribute.
	 *
	 * @param      Like $l Like
	 * @return     void
	 * @throws     PropelException
	 */
	public function addLike(Like $l)
	{
		if ($this->collLikes === null) {
			$this->initLikes();
		}
		if (!in_array($l, $this->collLikes, true)) { // only add it if the **same** object is not already associated
			array_push($this->collLikes, $l);
			$l->setPhoto($this);
		}
	}
	 ...
	 * been saved, it will retrieve related Likes from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in Photo.
	 */
	public function getLikesJoinUser($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(PhotoPeer::DATABASE_NAME);
		}
		...
		if ($this->collLikes === null) {
			if ($this->isNew()) {
				...
				$this->collLikes = LikePeer::doSelectJoinUser($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	...
	public function countPhotoComments(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		...
		if ($this->collPhotoComments === null) {
			if ($this->isNew()) {
				...
				$count = PhotoCommentPeer::doCount($criteria, false, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
	...
	public function getPhotoCommentsJoinUser($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		...
		if ($this->collPhotoComments === null) {
			if ($this->isNew()) {
				...
				$this->collPhotoComments = PhotoCommentPeer::doSelectJoinUser($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	...
	/**
	 * Clears out the collUserPhotos collection (array).
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	 ...
	 * @see        addUserPhotos()
	 */
	public function clearUserPhotos()
	{
		$this->collUserPhotos = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collUserPhotos collection (array).
	 *
	 * By default this just sets the collUserPhotos collection to an empty array (like clearcollUserPhotos());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initUserPhotos()
	{
		$this->collUserPhotos = array();
	}

	/**
	 * Gets an array of UserPhoto objects which contain a foreign key that references this object.
	 *
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Photo has previously been saved, it will retrieve
	 * related UserPhotos from storage. If this Photo is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	 ...
	 * @throws     PropelException
	 */
	public function getUserPhotos($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(PhotoPeer::DATABASE_NAME);
		}
		...
		if ($this->collUserPhotos === null) {
			if ($this->isNew()) {
			   $this->collUserPhotos = array();
				...
				$criteria->add(UserPhotoPeer::PHOTO_ID, $this->id);

				UserPhotoPeer::addSelectColumns($criteria);
				$this->collUserPhotos = UserPhotoPeer::doSelect($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				UserPhotoPeer::addSelectColumns($criteria);
				if (!isset($this->lastUserPhotoCriteria) || !$this->lastUserPhotoCriteria->equals($criteria)) {
					$this->collUserPhotos = UserPhotoPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastUserPhotoCriteria = $criteria;
		return $this->collUserPhotos;
	}

	/**
	 * Returns the number of related UserPhoto objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related UserPhoto objects.
	 * @throws     PropelException
	 */
	public function countUserPhotos(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(PhotoPeer::DATABASE_NAME);
		} else {
		...
		if ($this->collUserPhotos === null) {
			if ($this->isNew()) {
				...
				$count = UserPhotoPeer::doCount($criteria, false, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				if (!isset($this->lastUserPhotoCriteria) || !$this->lastUserPhotoCriteria->equals($criteria)) {
					...
					$count = count($this->collUserPhotos);
				}
			} else {
				$count = count($this->collUserPhotos);
			}
	...
	/**
	 * Method called to associate a UserPhoto object to this object
	 * through the UserPhoto foreign key attribute.
	 *
	 * @param      UserPhoto $l UserPhoto
	 * @return     void
	 * @throws     PropelException
	 */
	public function addUserPhoto(UserPhoto $l)
	{
		if ($this->collUserPhotos === null) {
			$this->initUserPhotos();
		}
		if (!in_array($l, $this->collUserPhotos, true)) { // only add it if the **same** object is not already associated
			array_push($this->collUserPhotos, $l);
			$l->setPhoto($this);
		}
	}
	 ...
	 * Otherwise if this Photo is new, it will return
	 * an empty collection; or if this Photo has previously
	 * been saved, it will retrieve related UserPhotos from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in Photo.
	 */
	public function getUserPhotosJoinUser($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(PhotoPeer::DATABASE_NAME);
		}
		...
		if ($this->collUserPhotos === null) {
			if ($this->isNew()) {
				...
				$this->collUserPhotos = UserPhotoPeer::doSelectJoinUser($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	...
	public function clearAllReferences($deep = false)
	{
		if ($deep) {
			if ($this->collLikes) {
				foreach ((array) $this->collLikes as $o) {
			...
			if ($this->collUserPhotos) {
				foreach ((array) $this->collUserPhotos as $o) {
					$o->clearAllReferences($deep);
				}
			}
		} // if ($deep)
